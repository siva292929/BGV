...
import {
  ...
  MapPin, Shield, LogOut, ChevronLeft, ChevronRight, FileCheck, Loader2, PenTool, FileText, ArrowRight, Info, ExternalLink, Smartphone
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';

const HybridField = ({ label, dbValue, fileKey, subLabel, files, setFiles }) => (
    ...
    <div className="flex justify-between items-start mb-4">
      ...
      {dbValue && (
        <div className="flex items-center gap-1.5 text-[10px] font-bold text-indigo-600 bg-white border border-indigo-100 px-3 py-1 rounded-full shadow-sm">
          <CheckCircle size={12} className="text-indigo-500 fill-indigo-200/50" /> AUTO-VERIFIED
        </div>
      )}
    </div>
...
const CandidatePortal = () => {
  const [currentStep, setCurrentStep] = useState(0); // Start at 0 for Phone Verification
    ...
    const checkUserStatus = async () => {
      try {
        ...
        if (['Under Review', 'Verified', 'Rejected'].includes(res.data.status)) {
          ...
          setCurrentStep(1); // Jump to Stage 1 if already verified
        }
      } catch (err) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Link phone to user on backend and update local auth state
      await checkAuth();
      await handleAutoFetch(phoneNumber.trim());
      setCurrentStep(1);
    } catch (err) {
      // Prioritize the error message from the server
  ...
  const nextStep = () => {
    // Stage 0: Verification handled via handleVerifyOtp button
    ...
    if (currentStep === 1) {
      ...
      if (!aadharSatisfied || !panSatisfied) {
        return alert("Please ensure both Aadhaar and PAN details are provided (Autofetched or Uploaded).");
      }
    }
    ...
    if (currentStep === 3 && !isFresher) {
      ...
      const paySatisfied = autoData?.payslip || files.payslip;
      if (!expSatisfied || !paySatisfied) {
        return alert("Please provide professional documents or select 'I am a Fresher'.");
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] p-4 md:p-8 lg:p-12">
      <div className="max-w-6xl mx-auto bg-white/80 backdrop-blur-2xl shadow-[0_40px_100px_-20px_rgba(0,0,0,0.1)] rounded-[56px] overflow-hidden border border-white">
        ...
        <div className="bg-slate-950 px-10 py-8 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <span className="block font-black text-white text-lg tracking-widest uppercase">DarwinTrace</span>
        ...
        <div className="p-8 md:p-16 lg:p-20">
          <div className="min-h-[500px]">
            ...
            {/* STAGE 0: PHONE VERIFICATION & RECORD LINKING */}
            {currentStep === 0 && (
              <div className="max-w-xl mx-auto py-12 space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-slate-500 font-medium leading-relaxed">Enter your registered mobile number to bridge your records from the DarwinTrace Global Database.</p>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  {!otpSent ? (
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <button onClick={() => setOtpSent(false)} className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-indigo-600 transition-all">Use Different Number</button>
                    </div>
                  )}
                </div>
              </div>
            ...
            {currentStep === 1 && (
              <div className="space-y-12 animate-in slide-in-from-right-16 duration-700">
                <div className="space-y-2">
                  ...
                  <p className="text-slate-500 font-medium">Autofetched records detected. Please upload any missing documents.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <HybridField
                    ...
                    fileKey="aadhar"
                    subLabel="Upload Aadhaar Scan (PDF/JPG)"
                  ...
                  <HybridField
                    ...
                    fileKey="pan"
                    subLabel="Upload PAN Card Scan (PDF/JPG)"
                    files={files} setFiles={setFiles}
                  />
                </div>
              </div>
            ...
            {currentStep === 2 && (
              <div className="space-y-12 animate-in slide-in-from-right-16 duration-700">
                <div className="space-y-2">
                  ...
                  <p className="text-slate-500 font-medium">Educational history records fetched from Master Database.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <HybridField label="10th Marksheet" dbValue={autoData?.tenthPercentage ? `${autoData.tenthPercentage}%` : null} fileKey="tenth" subLabel="Upload Class 10th PDF" files={files} setFiles={setFiles} />
                  <HybridField label="12th Marksheet" dbValue={autoData?.twelfthPercentage ? `${autoData.twelfthPercentage}%` : null} fileKey="twelfth" subLabel="Upload Class 12th PDF" files={files} setFiles={setFiles} />
                  <HybridField label="Bachelor's Degree" dbValue={autoData?.degreeGPA ? `${autoData.degreeGPA} CGPA` : null} fileKey="degree" subLabel="Upload Degree PDF" files={files} setFiles={setFiles} />
                </div>
              </div>
            )}
              ...
              <div className="space-y-12 animate-in slide-in-from-right-16 duration-700">
                ...
                {!isFresher && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in zoom-in duration-500">
                    <HybridField label="Previous Employment" dbValue={autoData?.experience} fileKey="experience" subLabel="Relieving Letter / Service Certificate" files={files} setFiles={setFiles} />
                    <HybridField label="Financial Proof" dbValue={autoData?.payslip} fileKey="payslip" subLabel="Last 3 Months Salary Slips" files={files} setFiles={setFiles} />
                  </div>
                )}
              </div>
            )}
                ...
                <div className="max-w-4xl mx-auto">
                  {!showPreview ? (
                    ...
                    <div className="space-y-12">
                      {/* LEGALLY BINDING DOCUMENT PREVIEW */}
                            ...
                            <div className="space-y-8 text-base md:text-lg leading-relaxed text-slate-700">
                              <div className="grid grid-cols-2 gap-8 bg-slate-50 p-8 rounded-3xl font-sans mb-10">
                                <div>
                                  ...
                                  <p className="font-bold text-slate-900">{autoData?.fullName || user?.name || "Verified User"}</p>
                                </div>
                                <div>
                              ...
                              <p className="text-justify font-medium italic">
                                "I hereby grant full permission to DarwinTrace and its authorized verification agents to conduct a comprehensive background screening. I declare that the electronic records fetched and manual documents uploaded are authentic and represent my genuine history."
                              </p>
                            </div>
            ...
            {currentStep === 5 && (
              <div className="text-center py-20 px-8 space-y-12 animate-in zoom-in-95 duration-1000">
                ...
                <div className="space-y-6">
                  <h2 className="text-6xl font-black text-slate-900 tracking-tighter leading-none">Verification Locked!</h2>
                  <div className="max-w-lg mx-auto p-10 bg-indigo-50 border-2 border-indigo-100 rounded-[48px] space-y-4">
                    <p className="text-indigo-900 font-black uppercase tracking-widest text-sm">System Update</p>
                ...
                <button
                  ...
                  className="px-12 py-5 bg-slate-950 text-white rounded-[24px] font-black text-sm uppercase tracking-[0.3em] hover:scale-105 transition-all shadow-2xl shadow-slate-200"
                >
                  Exit Portal
                </button>
              </div>
          ...
          {currentStep >= 1 && currentStep < 5 && !showPreview && (
            <div className="flex flex-col md:flex-row justify-between items-center mt-24 pt-12 border-t-2 border-slate-50 gap-8">
              {currentStep > 1 ? (
                <button
                  onClick={prevStep}
      ...
      <p className="text-center mt-12 text-slate-400 font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2">
        <Shield size={14} /> Encrypted by DarwinTrace Security Engine
      </p>
    </div>
  );
};
